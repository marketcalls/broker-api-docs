...
- Generated at: 2026-07-21 07:54:25 UTC
...
<section class="version-hero">
  ...
  <p><strong>Nubra's most powerful OMS is here with REST API V3.</strong> This documentation covers authentication, market data, trading, portfolio, and realtime order workflows using the current V3 request and response payloads.</p>
  <p>Use the PROD base URL <code>https://api.nubra.io</code> for live production usage and the UAT base URL <code>https://uatapi.nubra.io</code> for sandbox testing.</p>
</section>
...
<a id="page-index--key-sections"></a>
## Key Sections

If you are getting started, start with these pages first:

1. [Authentication](#page-authentication)
2. [Static IP Validation](#page-static-ip-validation)
3. [Orders Overview](#page-trading-orders-orders)
4. [Place Order](#page-trading-orders-place-order)
5. [Place Multi Order](#page-trading-orders-place-basket-order)
6. [Place Strategy Order](#page-trading-orders-place-flexi-order)
7. [Get Order Margin](#page-trading-orders-get-margin)
8. [Realtime Order Updates](#page-trading-realtime-order-updates)

---

<a id="page-index--quick-start"></a>
## Quick Start

For a new REST API V3 integration, use this order:

1. Complete [Authentication](#page-authentication) using the environment you are integrating with: PROD for live usage or UAT for sandbox testing.
2. Call [Static IP Validation](#page-static-ip-validation) if static IP access is enabled for the account.
3. Use [Get Instruments](#page-get-instruments) to resolve the `ref_id` and exchange metadata needed by the V3 payloads.
4. Read [Orders Overview](#page-trading-orders-orders) to understand the V3 request model and intent-order flow.
5. Run one primary placement flow using [Place Order](#page-trading-orders-place-order) or [Place Strategy Order](#page-trading-orders-place-flexi-order), depending on your integration.
6. Use [Get Order Margin](#page-trading-orders-get-margin) and [Realtime Order Updates](#page-trading-realtime-order-updates) to complete your order workflow.
...
<section class="version-hero">
  ...
  <p>This authentication guide is for users working on REST API accounts already mapped to OMS V3.</p>
  <p>Use the environment URL that matches the account mapping you have been enabled on. In most rollout cases, teams validate first on UAT and then switch the same V3 flow to PROD after the production account has been mapped.</p>
</section>
...
> If static IP access is enabled for the account, call [Static IP Validation](#page-static-ip-validation) after login to verify the current outbound IP before sending trading requests.
...
> This endpoint returns aÂ **TOTP secret**.
> 

     The client must save this secret andÂ add it to an Authenticator AppÂ (e.g., Google      Authenticator or any other preferred TOTP-compatible app).
...
<section class="version-hero">
  ...
  <p>These usage limits are the operating baseline for REST API accounts mapped to OMS V3.</p>
  <p>Use this page while testing the newer V3 payload flow in the environment where your account has been mapped.</p>
</section>
...
Source: docs/rest-api-v3/static-ip-validation.md

<a id="page-static-ip-validation"></a>
<a id="page-static-ip-validation--static-ip-validation"></a>
...
<section class="version-hero">
  ...
  <p>Use this endpoint to verify whether the current outbound IP matches the static IPs registered for the account.</p>
  <p>The Python SDK performs this validation automatically. REST API users can verify static IP access using this API after authentication.</p>
</section>

<a id="page-static-ip-validation--endpoint"></a>
## Endpoint
...
<a id="page-static-ip-validation--base-urls"></a>
## Base URLs
...
<a id="page-static-ip-validation--headers"></a>
## Headers
...
<a id="page-static-ip-validation--curl"></a>
## cURL
...
<a id="page-static-ip-validation--response"></a>
## Response
...
<a id="page-static-ip-validation--response-fields"></a>
## Response Fields
...
<a id="page-static-ip-validation--usage-notes"></a>
## Usage Notes
...
<a id="page-static-ip-validation--error-example"></a>
## Error Example
...
<section class="version-hero">
  ...
  <p>Use this instrument master page as the starting point for REST API accounts mapped to OMS V3.</p>
  <p>The returned <code>ref_id</code>, exchange metadata, and underlying fields here feed directly into the V3 order, market-data, and realtime payload examples across this documentation set.</p>
</section>
...
<a id="page-get-instruments--base-urls"></a>
## Base URLs
...
<a id="page-market-data-bhavcopy--base-urls"></a>
## Base URLs
...
<a id="page-market-data-current-price--base-urls"></a>
## Base URLs
...
> âš ï¸ When `exchange` is `NSE` or not provided, the default endpoint is used without query parameters.
...
<a id="page-market-data-market-quotes--base-urls"></a>
## Base URLs
...
=== "NSE"
    ...
    ```bash
    curl --location --globoff 'https://api.nubra.io/orderbooks/{nse_ref_id}?levels=5' \
    --header 'x-device-id: TS123' \
    --header 'Authorization: Bearer eyJh...6Pno'
    ```
...
=== "BSE"
    ...
    ```bash
    curl --location --globoff 'https://api.nubra.io/orderbooks/{bse_ref_id}?levels=5' \
    --header 'x-device-id: TS123' \
    --header 'Authorization: Bearer eyJh...6Pno'
    ```
...
=== "MCX"
    ...
    ```bash
    curl --location --globoff 'https://api.nubra.io/orderbooks/{mcx_ref_id}?levels=5' \
    --header 'x-device-id: TS123' \
    --header 'Authorization: Bearer eyJh...6Pno'
    ```
...
<a id="page-market-data-historical-market-data--base-urls"></a>
## Base URLs
...
Note:
Intervals less than 1 day â†’ data for last 3 months
Intervals 1 day or more â†’ data for up to 10 years (stocks)
...
<a id="page-market-data-option-chain--base-urls"></a>
## Base URLs
...
<a id="page-market-data-realtime-data--get-websocket-url"></a>
## Get WebSocket URL

After login, call the user info API with the same `session_token` and `x-device-id`.
...
Use `env_info.market_ws_url` from the response as the realtime market-data WebSocket URL.

```json
{
  "env_info": {
    "market_ws_url": "wss://api.nubra.io/apibatch/ws"
  }
}
```
...
<section class="version-hero">
  ...
  <p>This is the REST V3 trading track for accounts mapped to OMS V3.</p>
  <p>Use these pages when validating the newer V3 intent-order payload structure in the environment where your account has been mapped.</p>
</section>
...
Authenticated REST requests in this V3 track should include `Authorization` and `x-device-id`. Include `Content-Type: application/json` when the request sends a JSON body.
...
!!! important "stratTags"
    A single-order payload can pass exactly one tag inside `stratTags`. Do not use underscores. Use a hyphenated or plain tag such as `rest-api-v3-basic-usage`.
...
<a id="page-trading-orders-place-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-place-order--headers"></a>
## Headers
...
For `stratTags`, pass exactly one tag inside the list and do not use underscores. Use a hyphenated or plain tag such as `rest-api-v3-basic-usage`.
...
=== "Tagged Limit Order"
    ...
    Use this pattern when the order should later be grouped or retrieved through client-defined strategy labels. These tags are returned in the create response and can also be used with [Get Orders](#page-trading-orders-get-order) query filters.
...
| `orders[].stratTags` | array | Optional | Client-defined strategy tag for grouping and traceability. Pass exactly one tag inside the list. Do not use underscores; use hyphenated or plain tags such as `abc-def` |
...
| `stratTags` | Client-defined strategy tag echoed back for downstream grouping and retrieval |
...
- Do not use underscores in `stratTags`. Use hyphenated or plain tags instead, for example `abc-def` instead of `abc_def`.
...
!!! important "stratTags"
    Each order item can pass exactly one tag inside `stratTags`. Do not use underscores. Use a hyphenated or plain tag such as `rest-api-v3-multi-order-buy`.
...
<a id="page-trading-orders-place-basket-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-place-basket-order--headers"></a>
## Headers
...
<a id="page-trading-orders-place-basket-order--curl"></a>
## cURL
...
<a id="page-trading-orders-place-flexi-order--endpoint"></a>
## Endpoint
...
<a id="page-trading-orders-place-flexi-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-place-flexi-order--headers"></a>
## Headers
...
<a id="page-trading-orders-place-flexi-order--curl"></a>
## cURL
...
<a id="page-trading-orders-place-flexi-order--example-order-patterns"></a>
## Example Order Patterns
...
!!! important "Multi-leg execution mode"
...
!!! note "Strategy side and leg direction"
...
| `orders[].stratTags` | array | no | single string inside list | Strategy or tracking tag. Pass exactly one tag inside the list. Do not use underscores; use hyphenated or plain tags such as `abc-def`. |
...
| `orders[].stratTags` | array | Strategy tag echoed back inside a one-item list. |
...
!!! important-rules "Important Rules"
    ...
    - Use [Get Order Margin](#page-trading-orders-get-margin) before placement when margin impact matters.

<a id="page-trading-orders-place-flexi-order--what-to-read-next"></a>
## What To Read Next

1. [Get Orders](#page-trading-orders-get-order)
2. [Modify Order](#page-trading-orders-modify-order)
3. [Cancel Order](#page-trading-orders-cancel-order)
4. [Get Order Margin](#page-trading-orders-get-margin)
...
<a id="page-trading-orders-modify-order--endpoint"></a>
## Endpoint
...
<a id="page-trading-orders-modify-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-modify-order--headers"></a>
## Headers
...
<a id="page-trading-orders-modify-order--curl"></a>
## cURL

```bash
curl --location 'https://api.nubra.io/sentinel/orders/modify' \
--header 'Authorization: Bearer <session_token>' \
--header 'x-device-id: <device_id>' \
--header 'Content-Type: application/json' \
--data '{
...
<a id="page-trading-orders-modify-multi-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-modify-multi-order--headers"></a>
## Headers
...
<a id="page-trading-orders-cancel-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-cancel-order--headers"></a>
## Headers
...
```bash
curl --location 'https://api.nubra.io/sentinel/orders/cancel' \
--header 'Authorization: Bearer <session_token>' \
--header 'x-device-id: <device_id>' \
--header 'Content-Type: application/json' \
--data '{
...
<a id="page-trading-orders-get-order--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-get-order--headers"></a>
## Headers
...
<a id="page-trading-orders-get-order--filter-by-intentorderid"></a>
...
<a id="page-trading-orders-get-order--filter-by-strattags"></a>
## Filter By `stratTags`

Use `stratTags` when you want to retrieve orders associated with one or more client-defined strategy labels.

```bash
curl --location 'https://api.nubra.io/sentinel/orders?stratTags=momentum,breakout' \
--header 'Authorization: Bearer <session_token>' \
--header 'x-device-id: <device_id>'
```

Pass several tags as a comma-separated list. This is useful when orders are created with shared `stratTags` and later need to be reviewed, reconciled, or tracked together.
...
<a id="page-trading-realtime-order-updates--get-websocket-url"></a>
## Get WebSocket URL
...
Use `env_info.user_ws_url` from the response as the order-update WebSocket URL.

```json
{
  "env_info": {
    "user_ws_url": "wss://api-latest.nubra.io/ws"
  }
}
```
...
Connect to the `user_ws_url` returned by the user info API. After the WebSocket opens, send this text message:
...
<a id="page-trading-orders-get-margin--base-urls"></a>
## Base URLs
...
<a id="page-trading-orders-get-margin--headers"></a>
## Headers
...
```bash
curl --location 'https://api.nubra.io/sentinel/orders/funds_required' \
--header 'Authorization: Bearer <session_token>' \
--header 'x-device-id: <device_id>' \
--header 'Content-Type: application/json' \
--data '{
...
- Pass only one tag inside `stratTags` and do not use underscores.
...
<a id="page-trading-portfolio-holdings--base-urls"></a>
## Base URLs
...
<a id="page-trading-portfolio-holdings--request-payload"></a>
## Request Payload

This is a `GET` endpoint and does not require a request body.

<a id="page-trading-portfolio-holdings--response-structure"></a>
## Response Structure
...
<a id="page-trading-portfolio-holdings--response-attributes"></a>
## Response Attributes
...
<a id="page-trading-portfolio-holdings--important-rules"></a>
## Important Rules
...
<a id="page-trading-portfolio-positions--base-urls"></a>
## Base URLs
...
<a id="page-trading-portfolio-positions--request-payload"></a>
## Request Payload
...
<a id="page-trading-portfolio-positions--important-rules"></a>
## Important Rules
...
Fetch the current funds and margin snapshot for a V3 mapped account, including available cash, blocked margin, collateral, MTM values, brokerage, and withdrawal-related fields.
...
<a id="page-trading-portfolio-funds--base-urls"></a>
## Base URLs
...
```bash
curl --location 'https://api.nubra.io/sentinel/portfolio/user_funds_and_margin' \
--header 'Authorization: Bearer <session_token>' \
--header 'x-device-id: <device_id>'
```

<a id="page-trading-portfolio-funds--request-payload"></a>
## Request Payload
...
```json
{
  ...
  "portFundsAndMargin": {
    "clientCode": "XXXXX",
    "startOfDayFunds": 2888888,
    "payInCredit": 0,
    "payOutDebit": 0,
    "netDerivativePremBuy": 5612625,
    "netDerivativePremSell": 5538000,
    "netDerivativePrem": -74625,
    "cashBlockedCncTraded": 1868,
    "cashBlockedCncOpen": 0,
    "cashBlockedDerivOpen": 0,
    "cashCncTradedAndOpen": 1868,
    "mtmDeriv": -74625,
    "mtmEqIdayCnc": -555,
    "mtmEqDelivery": -222027,
    "netTradingAmount": 28680916,
    "netWithdrawalAmount": 23115099,
    "totalPayinCash": 0,
    "startOfDayCollateral": 0,
    "idayCollateralPledge": 0,
    "idayCollateralPledgeSell": 0,
    "totalCollateral": 0,
    "marginUsedDerivTraded": 0,
    "marginBlockDerivOpenOrder": 0,
    "marginUsedEqIday": 27817,
    "marginBlockedEqIdayOpen": 0,
    "netMarginAvailable": 28653099,
    "totalMarginBlocked": 27817,
    "derivativeMarginBlocked": 0,
    "brokerage": 18788
  }
}
```
...
| --- | --- | --- |
| `message` | `string` | Response message |
| `portFundsAndMargin.clientCode` | `string` | Trading account client code |
| `portFundsAndMargin.startOfDayFunds` | `number` | Opening funds balance |
| `portFundsAndMargin.netTradingAmount` | `number` | Net trading amount |
| `portFundsAndMargin.netWithdrawalAmount` | `number` | Net withdrawal amount |
| `portFundsAndMargin.totalCollateral` | `number` | Total collateral |
| `portFundsAndMargin.netMarginAvailable` | `number` | Currently available margin |
| `portFundsAndMargin.totalMarginBlocked` | `number` | Total blocked margin |
| `portFundsAndMargin.derivativeMarginBlocked` | `number` | Blocked derivative margin |
| `portFundsAndMargin.brokerage` | `number` | Brokerage field when present |

Important funds-level fields include:

| Field | Description |
| --- | --- |
| `portFundsAndMargin.payInCredit` | Funds credited through pay-in |
| `portFundsAndMargin.payOutDebit` | Funds debited through payout |
| `portFundsAndMargin.netDerivativePremBuy` | Option premium paid |
| `portFundsAndMargin.netDerivativePremSell` | Option premium received |
| `portFundsAndMargin.netDerivativePrem` | Net derivative premium |
| `portFundsAndMargin.cashBlockedCncTraded` | Cash blocked for CNC trades |
| `portFundsAndMargin.cashBlockedCncOpen` | Cash blocked for open CNC orders |
| `portFundsAndMargin.cashBlockedDerivOpen` | Cash blocked for open derivative orders |
| `portFundsAndMargin.cashCncTradedAndOpen` | Cash blocked across CNC traded and open orders |
| `portFundsAndMargin.mtmDeriv` | Derivative mark-to-market value |
| `portFundsAndMargin.mtmEqIdayCnc` | Intraday and CNC equity MTM |
| `portFundsAndMargin.mtmEqDelivery` | Delivery equity MTM |
| `portFundsAndMargin.totalPayinCash` | Total pay-in cash |
| `portFundsAndMargin.startOfDayCollateral` | Opening collateral value |
| `portFundsAndMargin.idayCollateralPledge` | Intraday collateral from pledged holdings |
| `portFundsAndMargin.idayCollateralPledgeSell` | Intraday collateral from sell-side pledge benefit |
| `portFundsAndMargin.marginUsedDerivTraded` | Used margin for derivative trades |
| `portFundsAndMargin.marginBlockDerivOpenOrder` | Blocked margin for derivative open orders |
| `portFundsAndMargin.marginUsedEqIday` | Used intraday equity margin |
| `portFundsAndMargin.marginBlockedEqIdayOpen` | Blocked intraday equity order margin |

<a id="page-trading-portfolio-funds--important-rules"></a>
## Important Rules

!!! important-rules "Important Rules"
    - Use this endpoint for V3 mapped accounts.
    - This endpoint is a funds and margin snapshot. The response does not auto-update after it is returned.
    ...
    - Use this endpoint for available-capital visibility, not as a substitute for pre-trade margin estimation.
    - Use [Get Order Margin](#page-trading-orders-get-margin) before order placement when you need request-specific margin requirements.
    - Use the environment that matches the mapped account. PROD and UAT funds are separate.
